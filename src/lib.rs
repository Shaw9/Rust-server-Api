...
pub fn send_notification(options: NotificationOptions) -> NotificationResult {
    ...
    use std::ptr;
    use std::thread;
    use windows::Win32::UI::Shell::{NIM_ADD, NIM_DELETE, NOTIFYICONDATAW, Shell_NotifyIconW};
    use windows::Win32::UI::WindowsAndMessaging::{GetDesktopWindow, IDI_INFORMATION, LoadIconW};
    ...
    let message_wide: Vec<u16> = options.message.encode_utf16().chain(iter::once(0)).collect();
    ...
    unsafe {
        ...
        // 设置通知图标类型
        nid.dwInfoFlags = windows::Win32::UI::Shell::NOTIFY_ICON_INFOTIP_FLAGS(NIIF_INFO);
    }
